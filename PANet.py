...
backbone = 'resnext50_32x4d'  # 'resnext50_32x4d'  # 'efficientnet-b4'  # 'resnet101'
...
model = segmentation_models.PAN(encoder_name=backbone, encoder_weights='imagenet', in_channels=3, classes=12,
                                activation=None, decoder_channels=256)
    ...
    if load_model:
        ...
        if generate_video_frames:
            ...
            for index in range(len(test_set)):
                ...
                plt.tight_layout()
                plt.savefig(f"./Results/Video_frames/PANet/{index}.jpg")

            if generate_video:
                image_folder = './Results/Video_frames/PANet'
